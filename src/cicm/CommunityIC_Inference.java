...
public class CommunityIC_Inference {
				...
				if(prev.size()>0){
					for(int k=0;k<nCommunities;k++){
						if(activationprob[k]<0)
							throw new RuntimeException(""+activationprob);
	...
	private void M_Step(boolean first_time){
		...
		for(int traceIndex=0;traceIndex<nTraces;traceIndex++){
			...
			for(Activation a: traces[traceIndex]){
				...
				if(prev.size()>0){
						...
						if(first_time){ 	//this happens in the initialization
							double [][]eta=computeEta(a, prev);
							for(int v_pos=0;v_pos<prev.size();v_pos++){
								Activation v_activation=prev.get(v_pos);
								int v=vertexId2Index.get(v_activation.userId);	
...
*/			
	...
	public static void main(String[] args) throws Exception {
		System.out.println("Community IC Inference (base)");
		System.out.println(CommunityIC_Inference.class.getSimpleName()
	...
	/*	String network="s1";
		...
		*/
		...
		CommunityIC_Inference inf=new CommunityIC_Inference();

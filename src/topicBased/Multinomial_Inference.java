...
public class Multinomial_Inference {
	...
	static double min_value=0.000001;
...
*/
	...
	private void M_Step() {
		...
		ArrayList<Activation>activationForTrace;
		for(int i=0;i<nTraces;i++){
			activationForTrace=propagationLog.getActionsForItem(traceSet[i]);
			for(int k=0;k<nCommunities;k++)
				phi_new[k][i]+=min_value;
			for(Activation a: activationForTrace){
				int vertexId=a.userId;
				int v=vertexId2Index.get(vertexId);
				for(int k=0;k<nCommunities;k++)
		...
		for(int k=0;k<nCommunities;k++)
			Weka_Utils.normalize(phi_new[k]);
	...
	public static void main(String[] args) throws Exception {
		System.out.println(Multinomial_Inference.class.getSimpleName()
		...
		*/
		...
		Multinomial_Inference mi=new Multinomial_Inference();
